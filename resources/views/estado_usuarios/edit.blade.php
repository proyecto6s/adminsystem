...
@section('content_header')
    <h1 class="text-center">EDITAR ESTADO DE USUARIO</h1>
@stop
    ...
    <div class="container d-flex justify-content-center">
        <div class="card shadow-sm p-4 mb-5 bg-white rounded" style="width: 50%;">
            <main class="mt-3">
                ...
                <form action="{{ route('estado_usuarios.update', $estado->COD_ESTADO) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="ESTADO" class="form-label">ESTADO</label>
                        <input type="text" name="ESTADO" id="ESTADO" class="form-control" value="{{ old('ESTADO', $estado->ESTADO) }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="DESCRIPCION" class="form-label">DESCRIPCIÓN</label>
                        <textarea name="DESCRIPCION" id="DESCRIPCION" class="form-control" required maxlength="255">{{ old('DESCRIPCION', $estado->DESCRIPCION) }}</textarea>
