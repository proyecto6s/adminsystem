...
<html>
<head>
    <title>Reporte de Equipos por Proyecto</title>
    <style>
         @page {
...
<body>
    ...
    <h1 class="text-center">Constructora Traterra S. de R.L</h1>
        <h1>Reporte de Equipos por Proyecto</h1>
        <p>{{ $fechaHora }}</p>
    </div>
        ...
        <tbody>
            @foreach ($asignaciones as $asignacion)
                ...
                <td>{{ $loop->iteration }}</td> <!-- Numeración automática -->
                    ...
                    <td>{{ $asignacion->proyectos ? $asignacion->proyectos->NOM_PROYECTO : 'Sin proyecto' }}</td>
                    <td>{{ $asignacion->DESCRIPCION }}</td>
                    <td>{{ $asignacion->FECHA_ASIGNACION_INICIO }}</td>
                    <td>{{ $asignacion->FECHA_ASIGNACION_FIN ?? 'PENDIENTE' }}</td> <!-- Mostrar "PENDIENTE" si no hay fecha -->
                </tr>
            @endforeach
            </tbody>
    </table>
